...
export default function ProfessionalJobCalendar({ user }) {
  ...
  const loadEvents = async () => {
    try {
      ...
      // Create events for in-progress jobs
      ...
      if (event.customer?.id) {
        await supabase
          ...
          .insert({
            ...
            content: `Job "${event.title}" has been rescheduled to ${new Date(formattedDate).toLocaleDateString()}${rescheduleForm.time ? ` at ${rescheduleForm.time}` : ''}`,
            job_id: jobId,
            created_by: user.id
          });
      }
        ...
        <>
                ...
                >
                  ...
                  <div className="mt-1 space-y-1">
                    {dayEvents.map(event => {
                      ...
                      if (event.customer?.full_name) {
                        displayTitle = `${event.title} (${event.customer.full_name.split(' ')[0]})`;
                      }
                      
                      const wasRescheduled = event.job?.last_rescheduled_at || false;
                      const rescheduledByPro = wasRescheduled && event.job?.last_rescheduled_by === user.id;
                      const rescheduledByCustomer = wasRescheduled && event.job?.last_rescheduled_by === event.job?.customer_id;
                      ...
                      return (
                        <div 
                          ...
                          className={`px-1.5 py-1 rounded text-xs md:text-sm truncate cursor-pointer ${
                            ...
                            rescheduledByPro ? 'border-purple-400' : 
                            rescheduledByCustomer ? 'border-orange-400' : ''
                          }`}
                        >
                          ...
                          {wasRescheduled && (
                            <span className="ml-1">
                              {rescheduledByPro ? '🔄' : rescheduledByCustomer ? '📅' : ''}
                            </span>
                          )}
                        </div>
                      );
          ...
          <div className="mt-4 flex flex-wrap gap-4 text-sm text-gray-600">
            ...
            <div className="flex items-center">
              ...
              <span>Rescheduled by Customer</span>
            </div>
          </div>
        </>
      )}
              ...
              <div className="space-y-4">
                ...
                {(selectedEvent.job?.service_type || selectedEvent.service_type) && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-gray-800">
                      {getServiceTypeLabel(selectedEvent.job?.service_type || selectedEvent.service_type)}
                    </span>
                  </div>
                )}
                ...
                {selectedEvent.customer && (
                  <div className="mt-4 pt-4 border-t border-gray-100">
                    <h4 className="text-sm font-medium text-gray-600 mb-2">Customer</h4>
                    <div className="flex items-center gap-3">
                      {selectedEvent.customer.avatar_url ? (
                        <img 
                          src={selectedEvent.customer.avatar_url}
                          alt="Customer"
                      ...
                      <div>
                        <p className="font-medium text-gray-800">{selectedEvent.customer.full_name}</p>
