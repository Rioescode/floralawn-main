...
export async function POST(request) {
  try {
    ...
    const { email, name, phone, address, area, ai_original_area, price, breakdown, preferredDate, customJobDetails } = body;
    ...
    // AI Tracking: Check if human had to correct the AI
    const wasManuallyCorrected = ai_original_area && ai_original_area !== area;
    const aiTrackingString = wasManuallyCorrected ? `[RLHF TRACKING: AI guessed ${ai_original_area} SQFT, but Human traced ${area} SQFT] ` : '';
      ...
      try {
        const { error: dbError } = await supabaseAdmin
          ...
          .insert([{
            ...
            custom_details: aiTrackingString + (customJobDetails || ''),
            status: 'NEW QUOTE',
      ...
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #f1f5f9; border-radius: 20px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.05); background-color: #ffffff;">
          ...
          <div style="padding: 35px;">
              ...
              <div style="background-color: #f8fafc; padding: 25px; border-radius: 15px; margin-bottom: 30px; border: 1px solid #f1f5f9;">
                  ...
                  <div style="margin-bottom: 15px; padding: 10px; background-color: #fef2f2; border: 1px solid #fca5a5; border-radius: 8px;">
                     <p style="margin: 0; font-size: 11px; font-weight: 900; color: #ef4444; text-transform: uppercase;">⚠️ AI Calibration Tracking (RLHF)</p>
                     <p style="margin: 3px 0 0 0; font-size: 12px; color: #991b1b;">The AI measured <strong>${ai_original_area.toLocaleString()} SQFT</strong>, but the user manually corrected the boundary to <strong>${area.toLocaleString()} SQFT</strong>.</p>
                  </div>
                  ` : ''}
