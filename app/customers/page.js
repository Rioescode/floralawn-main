...
export default function CustomersPage() {
  ...
  const handleBulkDeleteCustomers = async () => {
    ...
    if (!confirm(`Are you sure you want to delete ${selectedCustomers.length} customers? This will also delete their associated records. This cannot be undone.`)) return;
      ...
      const appointmentsChannel = supabase
        ...
        .on('postgres_changes', 
          ...
          (payload) => {
            ...
            // Notify for ALL contact form leads
            const isContactLead = 
              newLead.lead_source === 'contact_form' ||
              newLead.booking_type === 'Ready to Hire' ||
              newLead.booking_type === 'Contract Request';
            if (isContactLead) {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      // Count pending leads from contact form (by lead_source OR legacy booking_type)
      const pendingLeads = (data || []).filter(apt =>
        apt.status === 'pending' &&
        (
          apt.lead_source === 'contact_form' ||
          apt.booking_type === 'Ready to Hire' ||
          apt.booking_type === 'Contract Request'
        )
      );
          ...
          <div className="bg-white rounded-lg shadow-md mb-6">
            ...
            <div className="p-4 sm:p-6">
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {(() => {
                  // Filter to show ALL pending leads from contact forms
                  const pendingLeads = appointments.filter(apt =>
                    apt.status === 'pending' &&
                    (
                      apt.lead_source === 'contact_form' ||
                  ...
                  if (pendingLeads.length === 0) {
                    return <p className="text-sm text-gray-500 text-center py-4">No pending leads yet — they'll appear here when someone submits the contact form</p>;
                  }
                  
                  return pendingLeads.map((apt) => (
                    <div key={apt.id} className="border rounded-xl overflow-hidden bg-white border-blue-200 shadow-sm">
                      {/* Lead Header */}
                      <div className="p-4 bg-gradient-to-r from-blue-50 to-white border-b border-blue-100">
                        <div className="flex items-center gap-2 flex-wrap">
                          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-green-500 to-green-700 flex items-center justify-center text-white font-black text-sm flex-shrink-0">
                            {apt.customer_name?.[0]?.toUpperCase() || '?'}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="font-bold text-gray-900 text-sm">{apt.customer_name}</p>
                            <p className="text-xs text-gray-400">{apt.created_at ? new Date(apt.created_at).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }) : ''}</p>
                          </div>
                          <span className="px-2 py-0.5 text-xs font-bold bg-yellow-100 text-yellow-700 rounded-full">🕐 {apt.status}</span>
                        </div>
                      </div>

                      {/* Contact + Service Info */}
                      <div className="p-4 grid grid-cols-2 gap-2 text-xs border-b border-gray-100">
                        <div>
                          <p className="text-gray-400 font-bold uppercase tracking-wide mb-0.5">📞 Phone</p>
                          <a href={`tel:${apt.customer_phone}`} className="font-semibold text-green-600 hover:underline">{apt.customer_phone || '—'}</a>
                        </div>
                        <div>
                          <p className="text-gray-400 font-bold uppercase tracking-wide mb-0.5">📧 Email</p>
                          <a href={`mailto:${apt.customer_email}`} className="font-semibold text-blue-600 hover:underline truncate block">{apt.customer_email || '—'}</a>
                        </div>
                        <div className="col-span-2">
                          <p className="text-gray-400 font-bold uppercase tracking-wide mb-0.5">📍 Address</p>
                          <p className="font-semibold text-gray-800">{apt.address || (apt.street_address ? `${apt.street_address}, ${apt.city}` : apt.city) || '—'}</p>
                        </div>
                        <div>
                          <p className="text-gray-400 font-bold uppercase tracking-wide mb-0.5">🏗️ Service</p>
                          <p className="font-bold text-gray-900">{apt.service_type || '—'}</p>
                        </div>
                        <div>
                          <p className="text-gray-400 font-bold uppercase tracking-wide mb-0.5">📋 Estimate Pref.</p>
                          <p className="font-semibold text-gray-700">{apt.estimate_preference === 'meet_person' ? '🤝 In-Person' : apt.estimate_preference ? '📧 Email OK' : '—'}</p>
                        </div>
                      </div>

                      {/* Cleanup Assessment */}
                      {apt.cleanup_condition_level && (
                        <div className="px-4 py-3 bg-green-50 border-b border-green-100">
                          <p className="text-xs font-black text-green-700 uppercase tracking-wide mb-1">🍂 Cleanup Assessment</p>
                          <div className="flex items-center gap-3 flex-wrap text-xs">
                            {apt.cleanup_last_cleaned && <span className="text-gray-600">Last Cleaned: <strong>{apt.cleanup_last_cleaned}</strong></span>}
                            <span className="text-gray-600">Condition: <strong className="text-orange-600">{apt.cleanup_condition_level}/5 — {apt.cleanup_condition_label}</strong></span>
                          </div>
                          <div className="mt-1.5 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                            <div className="h-full rounded-full bg-gradient-to-r from-green-500 to-orange-500" style={{width: `${apt.cleanup_condition_level * 20}%`}} />
                          </div>
                        </div>
                      )}
                      ...
                      {apt.notes && (
                        <div className="px-4 py-3 border-b border-gray-100">
                          <p className="text-xs font-black text-gray-400 uppercase tracking-wide mb-1">💬 Message</p>
                          <p className="text-xs text-gray-700 italic leading-relaxed border-l-2 border-green-400 pl-2">"{apt.notes}"</p>
                        </div>
                      )}

                      {/* Media */}
                      {apt.media_urls && apt.media_urls.length > 0 && (
                        <div className="px-4 py-3 border-b border-gray-100 flex gap-2 flex-wrap">
                          {apt.media_urls.map((url, i) => (
                            <a key={i} href={url} target="_blank" rel="noreferrer" className="text-xs font-bold text-blue-600 bg-blue-50 border border-blue-200 rounded px-2 py-1 hover:bg-blue-100">
                              🖼️ Photo {i + 1}
                            </a>
                          ))}
                        </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    className="px-2 sm:px-3 py-1 text-xs font-medium text-red-700 bg-red-100 rounded-full hover:bg-red-200"
                  >
                    Bulk Delete
                  </button>
                  <button
