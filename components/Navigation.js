...
export default function Navigation() {
  ...
  return (
    <>
      <header 
        className={`fixed top-0 left-0 right-0 z-[1000] transition-all duration-500 bg-white/95 backdrop-blur-2xl border-b border-slate-200 ${
        ...
        <nav className="max-w-7xl mx-auto px-4 md:px-8">
          <div className="flex items-center justify-between">
            ...
            <div className="hidden lg:flex items-center space-x-2">
              {navigation.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className={`px-5 py-2.5 rounded-2xl text-sm font-black transition-all uppercase tracking-widest ${
                    pathname === item.href
              ...
              <div className="relative group/services">
                 <button className="px-5 py-2.5 rounded-2xl text-sm font-black text-slate-600 uppercase tracking-widest flex items-center gap-2">
                 ...
                 <div className="absolute top-full right-0 pt-4 opacity-0 invisible group-hover/services:opacity-100 group-hover/services:visible transition-all">
                    <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 p-6 min-w-[240px]">
            ...
            <div className="hidden lg:flex items-center space-x-6">
              <a href="tel:4013890913" className="bg-slate-900 text-white px-6 py-3 rounded-2xl font-black text-sm tracking-widest hover:bg-green-600 transition-all flex items-center gap-3">
                 <PhoneIcon className="w-5 h-5" /> (401) 389-0913
              </a>
              {user ? (
                <Link href={userRole === 'admin' ? '/admin/dashboard' : '/customer/dashboard'} className="w-10 h-10 bg-green-100 text-green-600 rounded-full flex items-center justify-center hover:bg-green-600 hover:text-white transition-all">
            ...
            <div className="space-y-6">
               {navigation.map((item) => (
                  ...
                  >
                     ...
                     <ArrowRightIcon className="w-8 h-8 text-green-600 group-active:text-white" />
                  </Link>
               ))}
                  ...
                  <div className="grid grid-cols-1 gap-4">
                     {services.slice(0, 5).map((s) => (
                        <Link key={s.name} href={s.href} onClick={() => setMobileMenuOpen(false)} className="text-xl font-bold text-slate-700 flex items-center gap-3">
                           <div className="w-2 h-2 rounded-full bg-green-500" /> {s.name}
                        </Link>
                     ))}
                  </div>
               </div>
